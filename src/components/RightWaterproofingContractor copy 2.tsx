        ...
        <div className="w-full xl:max-w-7xl xl:px-0">
          ...
          >
            ...
            <div className="w-full lg:w-1/2">
              ...
              >Message Passing Interface (MPI) is a library used for communication between
                processes in a distributed memory system. MPI is not a
                programming language; it is a programming model used for
                parallel programming, especially in clusters. In a cluster: The
                head node is called the master. The remaining nodes are called
                worker nodes. Using MPI, a large task is divided into smaller
                tasks and distributed among worker nodes. Each node works
                simultaneously, which increases speed and efficiency. Since this
                is a small module, only important and commonly used MPI concepts
                and functions are studied. More details can be learned from
                online resources. Why MPI? MPI is used because: It is a standard
                message passing library supported on all high-performance
                systems. MPI programs can run on different platforms without
                changing the source code. It supports parallel execution,
                allowing faster computation for large problems. It provides many
                useful library functions. There are multiple MPI implementations
                available. MPI Program To use MPI in a C program, include the
                header file: #include mpi.h MPI Program Structure An MPI program
                has both serial and parallel parts. General Structure: Include
                MPI header file Declarations and prototypes Program starts
                (Serial code) Initialize MPI environment (Parallel code starts)
                Perform computation and message passing Terminate MPI
                environment (Parallel code ends) Program ends (Serial code) 👉
                The parallel section begins with MPI_Init() and ends with
                MPI_Finalize(). Communicator A communicator is a group of
                processes that can communicate with each other. The default
                communicator is MPI_COMM_WORLD MPI_COMM_WORLD contains all
                processes Programmers can also create custom communicators with
                fewer processes Processes and Rank All processes belong to
                MPI_COMM_WORLD If there are p processes, their ranks range from:
                0 to p − 1 Each process is identified by its rank The master
                process always has rank 0
              </h1>
